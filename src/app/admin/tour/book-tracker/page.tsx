...
export default function BookingTracker_Desktop() {
  ...
  const filteredBookings = bookings.filter(booking => {
    // Search filter
    if (searchQuery && !booking.schoolName.toLowerCase().includes(searchQuery.toLowerCase())) {
      return false;
    }
    ...
    if (selectedStatus !== 'all') {
      ...
      if (booking.status !== statusMap[selectedStatus]) {
        return false;
      }
    }

    // State filter
    if (selectedState !== 'all' && booking.state.toLowerCase() !== selectedState.toLowerCase()) {
      return false;
    }

    // Cluster filter
    if (selectedCluster !== 'all' && booking.cluster !== selectedCluster) {
      return false;
    }
  ...
  const handleExport = () => {
    // CSV headers
    const headers = ['School Name', 'Cluster', 'State', 'LGA', 'Status', 'Last Email Sent', 'Booking Deadline', 'Booking Date', 'Updated By'];
    ...
    const rows = filteredBookings.map(booking => [
      ...
      booking.lga,
      booking.status,
      booking.lastEmailSent,
      booking.bookingDeadline,
      booking.bookingDate,
      booking.updatedBy,
    ]);
    ...
    link.setAttribute('download', `booking-tracker-${new Date().toISOString().split('T')[0]}.csv`);
          ...
          <div className="grid grid-cols-4 gap-3 mt-4 pt-4 border-t border-[#E5E7EB]">
            ...
            <div>
              ...
              >
                ...
                <option value="Lagos Central A">Lagos Central A</option>
                <option value="Lagos East B">Lagos East B</option>
              </select>
            </div>
            <div>
